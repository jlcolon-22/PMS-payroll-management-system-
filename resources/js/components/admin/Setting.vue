    ...
    <div class="flex">
        ...
        <div class="r w-full ">
                ...
                <button @click="profileShow" class="mr-3 relative" >
                    ...
                    <div v-show="profile" class="absolute bg-green-400 w-[200px] h-[150px] -right-1 top-11 pt-2 transition-all ease-in-out delay-150 flex flex-col justify-between z-30">
                        ...
                        <div class="flex justify-between">
                            <a href="" class="px-2 py-2 text-xs bg-green-700">Setting</a>
            ...
            <div class="w-full  relative">
               ...
               <div class="p-5">
                    <div class="shadow-lg  bg-[#0001] p-4 w-[400px] ml-10">
                     <h1 class="text-4xl font-bold font-sans p-4">Setting</h1>
                     <div class="mt-2">
                         <small class="font-bold">SSS</small>
                         <input type="text" v-model="sss" class="bg-white px-2 py-2 inline-block w-full" placeholder="sss">
                     </div>
                     <div class="mt-2">
                         <small class="font-bold">Pagibig</small>
                         <input type="text" v-model="pag_ibig" class="bg-white px-2 py-2 inline-block w-full" placeholder="Pagibig">
                     </div>

                     <div class="mt-4">
                         
                         <button @click="UpdateDeduction" class="bg-green-400 px-2 py-2 inline-block w-full">Save</button>
                     </div>
                </div>
               </div>
...
<script setup>
    ...
    const UpdateDeduction = (e) =>{
        e.preventDefault();

            axios.put(`/api/admin/setting/${id.value}/update`,{'sss':sss.value,'pag_ibig':pag_ibig.value})
                .then(res =>{
                    if(res.status == 200)
                    {
    ...
    const fetch = () =>{
        lazy.value = true;
        axios.get('/api/admin/setting').then(res=>{
                console.log(res.data[0])
   
               sss.value = res.data[0].sss;
               pag_ibig.value = res.data[0].pag_ibig;
